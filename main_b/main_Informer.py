...
for hierarchical_forecasting_method in hierarchical_forecasting_methods:
    ...
    for epoch in range(epochs):
        ...
        for step,(x,y,xt,yt) in enumerate(pbar):
            ...
            s = "[DOWN!]--train ==> Method: {} - Epoch: {} - Step: {} - Loss:{}".format(
    ...
    gt = output_cpu[0,:,-1]
    pd = y_cpu[0,:,-1]
    visual(gt, pd, './pic/'+'In_test.svg')

    torch.save(model.state_dict(), './checkpoint/' + 'In_checkpoint.pth')


    plt.figure()
    plt.plot(epoch_train_loss_list, label='Training Loss', color='blue', linewidth=2)
    plt.plot(epoch_test_loss_list, label='Validation Loss', color='orange', linewidth=2)
    plt.legend()
    plt.xlabel('Epoch')
    plt.ylabel('MSE Loss')
    plt.savefig('./result/' + 'In_loss.svg',
