...
# model | data
from model.ST_htqf_hier.ST_hier_main import ST_htqf_hier_Autoformer, ST_htqf_hier_Crossformer_Freq, ST_htqf_hier_LSTM, \
    ST_htqf_hier_Crossformer, ST_htqf_hier_TCN, ST_triformer, ST_htqf_hier_GRU, triformer, ST_htqf_hier_Informer
...
model_dict = {
            'Autoformer': ST_htqf_hier_Autoformer,
            'Crossformer': ST_htqf_hier_Crossformer,
            'Informer': ST_htqf_hier_Informer,
            "GRU": ST_htqf_hier_GRU,
            'TCN': ST_htqf_hier_TCN,
            'LSTM': ST_htqf_hier_LSTM,
            'ST_Triformer': ST_triformer,
            'Triformer': triformer,
            'Crossformer_freq': ST_htqf_hier_Crossformer_Freq
        }
...
optimizer_ADM = torch.optim.Adam(model.parameters(), lr=args.lr)
#optimizer_SDG = torch.optim.SGD(model.parameters(), lr=args.lr)
#optimizer = SAM(model.parameters(), base_optimizer=torch.optim.Adam, rho=0.5,lr=args.lr, weight_decay=1e-5)
...
def vail(vali_data, vali_loader, criterion):
    ...
    with torch.no_grad():
        for step, (seq_x, seq_x_y, seq_x_mark, seq_x_y_mark, seq_y, matrix_S,matrix_G,seq_xforq) in enumerate(vali_loader):
            ...
            output = model(seq_x, seq_x_mark, dec_y, seq_x_y_mark, tau, tau_norm, matrix_S,vali_data,matrix_G)
            true = seq_y
            if args.loss_fun=="mse":
...
for epoch in range(args.epochs):
    ...
    for step,(seq_x, seq_x_y, seq_x_mark, seq_x_y_mark, seq_y, matrix_S, matrix_G, seq_xforq) in enumerate(train_loader):
        ...
        output = model(seq_x, seq_x_mark, dec_y, seq_x_y_mark,tau, tau_norm, matrix_S,train_dataset,matrix_G)
        #print(output.shape)
        #exit()
        true = seq_y
        ...
        output = model(seq_x, seq_x_mark, dec_y, seq_x_y_mark, tau, tau_norm, matrix_S, train_dataset)
        true = seq_y
        if args.loss_fun == "mse":
            loss = criterion(output, true)
        elif args.loss_fun == "htqf":
    ...
    adjust_learning_rate(optimizer_ADM, epoch + 1,args.lr)
    ...
    with torch.no_grad():
        for step, (seq_x, seq_x_y, seq_x_mark, seq_x_y_mark, seq_y, matrix_S,matrix_G,seq_xforq) in enumerate(vali_loader):
            ...
            if args.loss_fun=="mse":
                ...
                SG = np.matmul(S, G)
                SG_T = torch.tensor(SG)
                SG_T = SG_T.float().to(args.device)
        ...
        if args.loss_fun=="mse":
            ...
            f = open(file_path, 'a')
            f.write("\n")
            f.write("========== {}_{}_{}_{} results ==========".format(args.model,args.hier_method,args.method,args.loss_fun))
            f.write("\n")
            ...
            for tau_n in range(3):
                ...
                f = open(file_path, 'a')
                f.write("\n")
                f.write("========== {}_{}_{}_{} results ==========".format(args.model, args.hier_method, args.method,
                                                                           args.loss_fun))
                f.write("\n")
