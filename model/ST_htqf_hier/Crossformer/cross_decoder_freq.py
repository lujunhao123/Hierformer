...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, seg_len, d_model, n_heads, d_ff=None, dropout=0.1, out_seg_num = 10, factor = 10):
        ...
        self.self_attention = TwoStageAttentionLayer(out_seg_num, factor, d_model, n_heads, \
                                d_ff, dropout)    
        self.cross_attention = AttentionLayer(FullAttention(scale=None, attention_dropout = dropout), d_model, n_heads, dropout = dropout)
        ...
        self.linear_pred = nn.Linear(d_model, d_model)

    def forward(self, x, cross, attn_mask=None, tau=None, delta=None):
        '''
        ...
        batch = x.shape[0]
        x = self.self_attention(x, attn_mask=None, tau=tau, delta=delta)
        x = rearrange(x, 'b ts_d out_seg_num d_model -> (b ts_d) out_seg_num d_model')
        
        cross = rearrange(cross, 'b ts_d in_seg_num d_model -> (b ts_d) in_seg_num d_model')
        tmp = self.cross_attention(
            x, cross, cross, attn_mask=None, tau=None, delta=None
        )
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, attn_mask=None, tau=None, delta=None):
        ...
        # final_predict = rearrange(final_predict, 'b (out_d seg_num) d_model -> b (seg_num d_model) out_d', out_d = ts_d)
        # 修改了
