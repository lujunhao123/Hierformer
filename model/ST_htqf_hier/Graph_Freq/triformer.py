...
class Triformer(nn.Module):
    def __init__(
        ...
        #print("configs.patch_sizes",configs.patch_sizes)
    ...
    def forward(self, batch_x,x, x_mark_enc, x_dec,x_mark_dec,tau,tau_norm, matrix_S,dataset):
        if self.notprinted:
        ...
        for layer, skip_generator in zip(self.layers, self.skip_generators):
            ...
            #print("skip_inp",skip_inp.shape)        # [4,1,9,scaler_len*d_model]   #
            skip = skip + self.custom_linear(skip_inp, weights[-1], biases[-1])
            #print("skip",skip.shape)                # [4,1,9,256]

        x = torch.relu(skip).squeeze(1)           # [4,9,256]
        predict_y = self.projections(x).transpose(2, 1) # [4,24,9]
...
class Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("data_concat",data_concat.shape)
...
class WeightGenerator(nn.Module):
    ...
    def forward(self):
        if self.factorized:
            ...
            # print("memory",memory.shape,"B",self.B[-1].shape) memory torch.Size([9, 1, 25]) B torch.Size([25, 128])
            # B[5**2,128] 在weights_generator_distinct men_dim==5
            bias = [
            ...
            #print("weights",weights[-1].shape,"bias",bias[-1].shape)
            # weights torch.Size([9, 128, 128]) bias torch.Size([9, 128].len(weights):2)
            return weights, bias
        else:
            #print("1")
...
if __name__ == '__main__':
    ...
    config = parse_args()
    input = torch.rand((4,96,9))
    model = Triformer(config)
    output = model(input)
    print(output.shape)
