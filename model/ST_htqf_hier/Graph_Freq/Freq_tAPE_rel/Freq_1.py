...
from model.ST_htqf_hier.Graph_Freq.Graph_Encoder.GFC_graphencoder import Graph_Encoder
...
class Impoveformer(nn.Module):
    def __init__(self,config):
        ...
        self.head_f1 = Flatten_Head(config=config,Multivariate=config.Multivariate_type,
                                         n_vars=config.enc_in, nf=self.head_nf_f,
                                         target_window=config.pred_len,
                                         head_dropout=0)
        self.head_f2 = Flatten_Head(config=config,Multivariate=config.Multivariate_type,
                                         n_vars=config.enc_in, nf=self.head_nf_f,
                                         target_window=config.pred_len,
                                         head_dropout=0)
    ...
    def forward(self,x, x_mark_enc, x_dec,x_mark_dec,tau,tau_norm, matrix_S,dataset):
...
class Flatten_Head(nn.Module):
    def __init__(self, config,Multivariate, n_vars, nf, target_window, head_dropout=0):
        super().__init__()

        self.Multivariate = Multivariate
        self.n_vars = n_vars

        if self.Multivariate=="Independent":
        ...
        elif self.Multivariate == "mix_MLP":
        ...
        elif self.Multivariate == "Graph":
            self.GraphEncoder = Graph_Encoder(config)
            self.linears1 = nn.ModuleList()
            # self.linears2 = nn.ModuleList()
            self.dropouts = nn.ModuleList()
            self.flattens = nn.ModuleList()
