...
from model.quantile.quannet import htqf_loss,htqf_loss_hier,sharq_1,sharq_33
########################################################################################################################
import warnings
import random
...
sharq_33 = sharq_33(tau, tau_norm, args)
...
# just for sharq33
def vail_sharq33(vali_data, vali_loader, criterion):
    model.eval()
    test_loss = []
    with torch.no_grad():
...
if args.loss_fun=="htqf":
    ...
    for epoch in range(args.epochs):
        ...
        for step,(seq_x, seq_x_y, seq_x_mark, seq_x_y_mark, seq_y, matrix_S, matrix_G, seq_xforq) in enumerate(train_loader):
            ...
            loss = sharq_33(output, true, matrix_S[0], matrix_G[0],train_dataset)
        ...
        print("sharq2 Epoch: {} cost time: {}".format(epoch + 1, time.time() - epoch_time))
        total_loss = np.average(train_loss)

        vali_loss = vail_sharq33(val_dataset,val_loader,sharq_33)
        test_loss = vail_sharq33(test_dataset,test_loader,sharq_33)
...
def _predict(vali_data, vali_loader,model_file,flag):
    ...
    with torch.no_grad():
        ...
        if args.loss_fun=="mse":
            preds1 = train_dataset.inverse_transform_y(preds)
            trues1 = train_dataset.inverse_transform_y(trues)
            mae, rmse, mape, _ ,_ = All_Metrics(preds1,trues1)
            ...
            for tau_n in range(3):
                picp, mpiw = All_PI2_Metrics(preds1[..., tau_n], preds1[..., -(tau_n + 1)], trues1)
                print("Average Horizon,PI {:.3f}, picp: {:.4f}%, mpiw: {:.2f}".format(
                    tau[0, -(tau_n + 1)] - tau[0, tau_n], picp * 100, mpiw))
                crps, cross_loss = All_PI1_Metrics(preds1, trues1[:, :, :, None])
